...
const Ads = () => {
  ...
  const fetchData = async (page = 1, limit = 10, term = '', sortField = '', sortOrder = '') => {
    try {
      ...
      const response = await axiosInstance.get(`/apis/ads`, {
        params: {
          search: term,
          page: page,
          limit: limit,
  ...
  const handleToggleStatus = async (adsId) => {
    ...
    const currentAds = ads.find((ad) => ad.adsId === adsId);
    const currentStatus = currentAds?.status; // Get the current status ('1' for active, '0' for inactive)
    ...
    try {
      const token = localStorage.getItem('token')
      // Toggle user status
      await axiosInstance.put(`/apis/toggleadstatus/${adsId}`)
  ...
  const handleEdit = async (id) => {
    ...
    try {
      const response = await axiosInstance.get(`/apis/adsdetails?adsId=${id}`)
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axiosInstance.put(`/apis/deletead/${id}`)
  ...
  const handleExport = async () => {
    try {
      // Making the API call
      const response = await axiosInstance.get(`/apis/ads/all`, {});
      // const response = await axiosInstance.get(`${backendUrl}/apis/ads/all`, {});
      ...
      if (response.data.status === 'true') {
        ...
        const formattedads = ads.map((ad , index) => {
          const { adsId ,photo , createdDate ,updatedDate , ...rest } = ad; // Destructure and remove unwanted fields
          return {
        ...
        const csvData = Papa.unparse(formattedads, {
          header: true,
        ...
        if (link.download !== undefined) { // Feature detection
          ...
          link.setAttribute('download', 'Ads.csv');  // CSV file name
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        ...
        // Update Ad
        response = await axiosInstance.put(`/apis/editads/${newAds.adsId}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
          timeout: 30000,
        ...
        // Add new Ad
        response = await axiosInstance.post(`/apis/addads`, formData, {
                              ...
                              <CCard>
                                <CCardBody>
                                  ...
                                  <CRow className="mt-2">
                                    ...
                                    <CCol md="6">
  <strong>URL:</strong> <a href={ads.url} target="_blank" rel="noopener noreferrer">Click here</a>
</CCol>
