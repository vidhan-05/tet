...
const UserList = () => {
  ...
  const fetchData = useCallback(async (page = 1, limit = 10, term = '', sortField = '', sortOrder = '') => {
    setIsReloading(true);
    try {
      // const response = await axiosInstance.get(`${backendUrl}/apis/getallusers`, {
      const response = await axiosInstance.get(`/apis/getallusers`, {
        params: {
          search: term,
          page: page,
          limit: limit,
    ...
    try {
      ...
      if (response.data.status === 'true') {
        ...
        const formattedUsers = users.map((user, index) => {
          const { userId, photo, contactNumberFlag, parentId, token, createdDate , updatedDate, familyMemberCount, ...rest } = user; // Destructure to exclude fields
          return {
        ...
        if (link.download !== undefined) { // Feature detection
          ...
          link.setAttribute('download', 'users.csv'); // CSV file name
...
<div className="button-container" style={{ display: 'flex', justifyContent: 'flex-start', gap: '10px', marginTop: '10px' }}>
  {!showForm && !isChildList && (
    <CButton color="primary" onClick={handleAddNewUser}>
      Add User
    </CButton>
  )}
              ...
              <CForm className="row g-3" onSubmit={handleFormSubmit}>
                ...
                <CCol md="6">
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    required={newUser.userType === 'family head' || (newUser.userType !== 'family head' && newUser.age >= 18)}
                    maxLength={10}
                  />
                                ...
                                <CCardBootstrap>
                                  <CCardBody>
                                    ...
                                    <CRow className="mt-2">
                                      ...
                                      <CCol md="6">
                                        <strong>Occupation:</strong> {user.occupation || 'NA'}
                                      </CCol>
                                    </CRow>
                                    <CRow className="mt-2">
                                      <CCol md="12">
