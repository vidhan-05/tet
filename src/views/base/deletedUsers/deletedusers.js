...
const UserList = () => {
  ...
  const fetchData = useCallback(async (page = 1, limit = 10, term = '', sortField = '', sortOrder = '' ) => {
    setIsReloading(true);
    try { 
    //   const response = await axiosInstance.get(`${backendUrl}/apis/deleted_users`, {
      const response = await axiosInstance.get(`/apis/deleted_users`, {
        params: {
          search: term,
          page: page,
          limit: limit,
      ...
      if (response.data && response.data.deletedUsers) {
        setUsers(response.data.deletedUsers);
        console.log(response.data.deletedUsers + " hello");
        setDisplayedUsers([]);
  ...
  const handleExport = async () => {
    try {
      ...
      if (response.data.status === 'true') {
        ...
        // Map 'status' field and exclude unwanted fields
        const formattedUsers = users.map(user => {
          const { userId, photo, contactNumberFlag, parentId, token, familyMemberCount, createdDate , updatedDate , ...rest } = user; // Destructure to exclude fields
          return {
            ...rest,
        ...
        if (link.download !== undefined) { // Feature detection
          ...
          link.setAttribute('download', 'familyHead.csv'); // CSV file name
  ...
  const handleUploadCSV = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.post(`/apis/addusersfromcsv`, formData, {
        headers: {
  ...
  const handleUploadCSVbyId = async (e) => {
    ...
    try {
      const response = await axiosInstance.post(
        `/apis/addusersfromcsvbyId?parentId=${parentId}`, // Use the state parentId here
  ...
  const handleToggleStatus = async (userId, parentId) => {
    ...
    const currentStatus = currentUser?.status; // Get the current status ('active' or 'inactive')
  
    // Determine the action based on the current status
    const action = currentStatus === 1 ? "deactive" : "active";
  ...
  const handleFormSubmit = async (event) => {
    ...
    // Append fields to formData, excluding undefined values
    Object.keys(newUser).forEach((key) => {
      // Skip undefined or empty values
      if (newUser[key] !== undefined && newUser[key] !== "") {
        formData.append(key, newUser[key]);
      } else {
        console.warn(`${key} is empty or undefined and was skipped`);
      }
    });
    ...
    try {
      ...
      // Determine API request based on editMode
      if (editMode) {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        // Update the users state with the new or edited user
        setUsers((prevUsers) =>
          editMode
  ...
  <CCol xs="auto" className="me-2">
    ...
    />
                ...
                {/* Conditional Buttons */}
                <div className="button-container" style={{ display: 'flex', justifyContent: 'flex-start', gap: '10px', marginTop: '10px' }}>
                {/* Import button for mode 'edit' and 'add'
                ...
                {!showForm && !isChildList && (
                  <>
                  {/* <CButton color="primary" onClick={handleAddNewUser} style={{ marginRight: '11px' }}>
                    Add User
                  </CButton> */}
                

                {/* Export button */}
                {/* <CButton color="primary" onClick={handleExport} style={{ marginRight: '11px' }}>
                  Export
                </CButton> */}
                </>
                )}
...
<CCol md="6">
  ...
  <CFormSelect
    id="userType"
    name="userType"
    value="family head" // Hardcoded to 'family head'
    onChange={handleInputChange} // You can still handle the input change, but it will not update the value of userType
    ...
    <option value="family head">Family Head</option>
                ...
                <CCol md="6">
                  ...
                  <CFormSelect
                    ...
                    required={newUser.userType !== 'family head' && editMode == true}
...
{/* Adding a CCol for Is Admin checkbox, aligning it to the right */}
<CCol md="6" className="d-flex justify-content-end align-items-end mt-3">
                      ...
                      {users.map((user, index) => (
                        <React.Fragment key={user.userId}>
                          <CTableRow>
                            ...
                            <CTableDataCell>{user.contactNumber || 'NA'}</CTableDataCell>
                            <CTableDataCell>{user.email || 'NA'}</CTableDataCell>
                            <CTableDataCell>
                              ...
                              <CCollapse visible={selectedUserId === user.userId}>
                                <CCardBootstrap>
                                  <CCardBody>
                                    ...
                                    <CRow className="mt-2">
                                      <CCol md="6">
                                        <strong>Contact Number:</strong> {user.contactNumber || 'NA'}
                                      </CCol>
                                      <CCol md="6">
                                    ...
                                    <CRow className="mt-2">
                                      <CCol md="6">
                                        <strong>Education:</strong> {user.education || 'NA'}
                                      </CCol>
                                      <CCol md="6">
                                    ...
                                    <CRow className="mt-2">
                                      ...
                                      <CCol md="6">
                                        <strong>Blood Group:</strong> {user.bloodGroup || 'NA'}
                                      </CCol>
                                    </CRow>
                                    <CRow className="mt-2">
                                      <CCol md="6">
